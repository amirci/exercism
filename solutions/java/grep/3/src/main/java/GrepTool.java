...
class GrepTool {
    ...
    String grep(String pattern, List<String> flags, List<String> files) {
        var strategy = SearchFiles.from(flags, files.size());
    ...
    private static class SearchFiles {
        static SearchFile from(List<String> flags, int fileCount) {
            var options = asSet(flags);
            var matcher = MatchStrategy.from(options);
            var formatter = GrepFormatter.from(options, fileCount);
        ...
        private static EnumSet<GrepOption> asSet(List<String> flags) {
            return flags.stream()
                    ...
                    .collect(() -> EnumSet.noneOf(GrepOption.class), EnumSet::add, EnumSet::addAll);
        }
    ...
    private interface GrepFormatter {
        ...
        static GrepFormatter from(EnumSet<GrepOption> options, int fileCount) {
            return line -> {
