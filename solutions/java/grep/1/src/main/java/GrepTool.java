...
class GrepTool {
    ...
    String grep(String pattern, List<String> flags, List<String> files) {
        var options = GrepOptions.parse(flags, files);

        return files.stream()
                .flatMap(file -> options.searchFile().search(file, pattern))
                .collect(Collectors.joining("\n"));
    }

    private record GrepOptions(SearchFile searchFile) {
        static GrepOptions parse(List<String> flags, List<String> files) {
            var options = asSet(flags);
            var matcher = MatchStrategy.from(options);
            var formatter = GrepFormatter.from(options, files);
            var searchFile = options.contains(GrepOption.FILES_ONLY)
                    ? SearchFile.filesWithMatches(matcher)
                    : SearchFile.matchingLines(matcher, formatter);

            return new GrepOptions(searchFile);
        }
    ...
    private interface SearchFile {
        ...
        static SearchFile matchingLines(MatchStrategy matcher, GrepFormatter formatter) {
            return (file, pattern) -> linesIn(file)
                    .filter(line -> matcher.matches(line.text(), pattern))
                    .map(formatter::format);
        }

        static SearchFile filesWithMatches(MatchStrategy matcher) {
