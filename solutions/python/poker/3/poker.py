...
def _card_values(hand: str) -> tuple[list[int], list[str]]:
    ...
    return [value for value, _ in cards], [suit for _, suit in cards]  # pylint: disable=disallowed-name
...
def best_hands(hands):
    ...
    best_score = max(score for _, score in scored_hands)  # pylint: disable=disallowed-name
