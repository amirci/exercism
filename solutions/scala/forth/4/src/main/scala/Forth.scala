...
private def initialContext = EvalContext(emptyStack, defaultOps)

extension (ctxt: EvalContext)
  def withNumbers(numbers: NumberStack): EvalState =
    Right(ctxt.copy(numbers = numbers))

private def updateNumbers(fn: UpdateFn[NumberStack])(ctxt: EvalContext): EvalState =
  ctxt.withNumbers(fn(ctxt.numbers))

private def parseNumber(maybeNum: String, ctxt: EvalContext): Option[EvalState] =
  maybeNum.toIntOption.map(nbr => updateNumbers(nbr :: _)(ctxt))

private def opWithTopTwo(fn: (a: Int, b: Int, rest: NumberStack) => NumberStack): EvalFn = { (ctxt: EvalContext) =>
  ctxt.numbers match {
...
private def opWithTop(fn: (a: Int, rest: NumberStack) => NumberStack): EvalFn = { (ctxt: EvalContext) =>
  ctxt.numbers match {
...
private def binaryOp(op: (Int, Int) => Int): EvalFn = opWithTopTwo { (a, b, rest) => op(b, a) :: rest }

private val swapTopTwo = opWithTopTwo { (a, b, rest) => b :: a :: rest }

private val copySecondOverFirst = opWithTopTwo { (a, b, rest) => b :: a :: b :: rest }

private def intDivisionWithGuards(ctxt: EvalContext): EvalState = ctxt.numbers match {
...
private def evalRegisteredOp(maybeOp: String, ctxt: EvalContext): Option[EvalState] =
  ctxt.ops
...
extension (iter: IterableOnce[String])
  def runProgram(init: EvalContext): EvalState =
    iter.iterator.foldLeft(Right(init): EvalState) { (state, token) => state.flatMap(parseAndEval(token)) }

private def isValidDefinitionName(newUserDef: String): Boolean = newUserDef.toIntOption.isEmpty

private def addUserDefined(newUserDef: String, program: List[String], ctxt: EvalContext): EvalState = {
...
private def evalUserDefinition(maybeUserDef: String, ctxt: EvalContext): Option[EvalState] = maybeUserDef
  .replace(";", "")
  .split("""\s+""")
  .toList match {
...
private def parseAndEval(token: String)(ctxt: EvalContext): EvalState =
  parseNumber(token, ctxt)
    .orElse(evalRegisteredOp(token, ctxt))
    .orElse(evalUserDefinition(token, ctxt))
