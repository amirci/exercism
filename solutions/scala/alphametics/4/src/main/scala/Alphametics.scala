object Alphametics {
  ...
  private case class Puzzle(columns: List[Column], leadingLetters: Set[Char])
  ...
  private def assignAddendDigits(puzzle: Puzzle, addends: List[Char], result: Char, remaining: List[Column], state: SolverState, sum: Int): Option[Assignments] =
    addends match
      case Nil =>
        assignResultDigit(puzzle, result, remaining, state, sum)
      case letter :: rest =>
        assignAddendDigit(puzzle, letter, rest, result, remaining, state, sum)
